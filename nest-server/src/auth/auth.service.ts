...
export class AuthService {
    ...
    async register(data: SignDto) {
        try {
            ...
            if (isDup) return new HttpException('Duplicate', HttpStatus.BAD_REQUEST)
    ...
    async login(data: LoginDto, setCookie: (token: string) => void) {
        ...
        if (!email && !numberPhone && !userName) {
            throw new HttpException({ msg: 'data not valid' }, HttpStatus.BAD_GATEWAY)
        }
        const user = await this.userModel
            ...
            .populate('posts')
        if (!user) throw new HttpException({ msg: 'Login fail' }, HttpStatus.UNAUTHORIZED)
        const { password: passHash, ...resUser } = user._doc
        const isPass = await compare(password, passHash)
        if (!isPass)
            throw new HttpException({ msg: 'Login fail' }, HttpStatus.UNAUTHORIZED)
    ...
    async loginFacebook(data: LoginFBDto, setCookie: (token: string) => void) {
        ...
        const passHash = await hash(uid, 10)
        if (!isEmail && !isTell) {
        ...
        const dataJwt = { userName: user.userName }
        const accessToken = this.signToken(dataJwt, '120s')
        const refreshToken = this.signToken(dataJwt, '7d')
        await this.redis.set(user.userName, refreshToken)
        setCookie(refreshToken)
