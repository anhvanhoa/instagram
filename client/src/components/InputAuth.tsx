    ...
    return (
        <div className='mx-10 mb-2'>
            <label className={classnames('relative flex items-center h-10 overflow-hidden group', { isValue: value })}>
                ...
                {isSuccess && <Icon className='absolute right-2 text-2xl text-green-700' icon='ei:check' />}
                {isError && <Icon className='absolute right-2 text-2xl text-red-700' icon='ei:close-o' />}
                <div></div>
            </label>
        </div>
    )
