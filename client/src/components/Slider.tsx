...
const Slider: React.FC<Props> = memo(({ children }) => {
    const dots: number[] = []
    const maxElemnt = Children.count(children)
        ...
        <div className='relative overflow-hidden'>
            ...
            <div
                className={classNames(
                    'absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-1',
                    {
                        hidden: maxElemnt === 1,
                    },
                )}
                ...
                {dots.map((_, index) => (
                    <div
                        ...
                        className={`w-[7px] h-[7px] rounded-[50%] transition-all ${
                            index === position ? 'bg-gray-600' : 'bg-gray-200'
                        }`}
                    ></div>
                ))}
            </div>
